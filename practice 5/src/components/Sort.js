...
const Sort = (props) => {
    ...
    const [localSort1, setLocalSort1] = useState(NONE);
    const [localSort2, setLocalSort2] = useState(NONE);
    const [localSort3, setLocalSort3] = useState(NONE);
    const [localDesc1, setLocalDesc1] = useState(false);
    const [localDesc2, setLocalDesc2] = useState(false);
    const [localDesc3, setLocalDesc3] = useState(false);

    const sort1 = sortState ? sortState.sort1 : localSort1;
    const sort2 = sortState ? sortState.sort2 : localSort2;
    const sort3 = sortState ? sortState.sort3 : localSort3;
    const desc1 = sortState ? sortState.desc1 : localDesc1;
    const desc2 = sortState ? sortState.desc2 : localDesc2;
    const desc3 = sortState ? sortState.desc3 : localDesc3;

    const update = (patch) => {
        if (onSortStateChange) {
            onSortStateChange({ sort1, sort2, sort3, desc1, desc2, desc3, ...patch });
        } else {
            if ('sort1' in patch) setLocalSort1(patch.sort1);
            if ('sort2' in patch) setLocalSort2(patch.sort2);
            if ('sort3' in patch) setLocalSort3(patch.sort3);
            if ('desc1' in patch) setLocalDesc1(patch.desc1);
            if ('desc2' in patch) setLocalDesc2(patch.desc2);
            if ('desc3' in patch) setLocalDesc3(patch.desc3);
        }
    };
